...
class JobLoader
{
     ...
     * @return int Verbleibendes Restgewicht des Auftrags im Lager/Pool
     * @throws Exception Bei Datenbankfehlern oder unvollständigen Daten
     */
    public function execute(int $truckId, int $orderId): int
    {
        ...
        try {
            ...
            if ($weightRemaining > $capacity || $weightRemaining < $weightTotal) {
                ...
                $loadedWeight = min($weightRemaining, $capacity);
                $remainingWeight = max(0, $weightRemaining - $loadedWeight);
